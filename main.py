...
import os
from preprocessing import facechop, histogram_eq, test
...
def read_data():
    ...
    for face in range(num_faces):
        ...
        for i, file in enumerate(os.listdir(face_path)):
            ...
            if(i <= 9):
                X_train[t_idx] = cv.resize(src = img, dsize= img_dim)
                Y_train[t_idx] = face
                t_idx = t_idx + 1
            else: 
                X_val[v_idx] = cv.resize(src = img, dsize= img_dim)
                v_idx = v_idx + 1
...
def preprocess(img):
    ...
    if len(img.shape) > 2:
        img = cv.cvtColor(img, cv.COLOR_RGB2GRAY)
    img = histogram_eq(img)
    img = cv.resize(src = img, dsize= img_dim)
    return img
...
# test = get_building()
try:
    test = plt.imread('yalefaces/01/subject01.gif')
